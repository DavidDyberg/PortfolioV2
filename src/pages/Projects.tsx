  ...
  return (
    <section className="container py-16 md:py-24">
      <div className="max-w-2xl mb-12 animate-fade-up">
        <div className="text-sm uppercase tracking-widest text-primary mb-3">Selected work</div>
      ...
      {!projects ? (
        ...
        <div className="text-center py-24 text-muted-foreground">No projects yet. Sign in as admin to add some.</div>
      ) : (
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map((p) => (
            <Link
            ...
            >
              <div className="aspect-[16/10] overflow-hidden bg-muted">
                {p.image_url ? (
                  <img src={p.image_url} alt={p.title} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-smooth" />
                ) : (
                  <div className="w-full h-full bg-gradient-primary opacity-30" />
                )}
              </div>
              <div className="p-6">
                <h3 className="text-xl font-semibold mb-2 group-hover:text-gradient">{p.title}</h3>
